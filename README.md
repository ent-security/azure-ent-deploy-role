...
A companion, read-only helper — **`list-ai-models.sh`** / **`list-ai-models.ps1`** —
surveys which Azure OpenAI models and how much TPM quota are available in a
subscription/region, to fill the deployment's model fields (see
...
Ent's Azure deployment runs inference through Azure OpenAI models provisioned in your
subscription: a **chat** model (the *Azure AI Model* field), a **fast** model (the
*Azure AI Fast Model* field), and an embedding model. Which models are offered — and
how much throughput (TPM quota) they have — depends on your subscription and region.

`list-ai-models.sh` / `list-ai-models.ps1` is a **read-only** helper that surveys a
subscription + region and prints the candidate models for each tier with their TPM
quota. It creates nothing and needs no existing Cognitive Services account, so it is
safe to run before or after `setup.sh`. Run it, pick a model per tier and a capacity
that fits under the free column, and tell Ent what to deploy.

```bash
# bash (macOS / Linux)
./list-ai-models.sh --subscription <your-subscription-id> --location eastus2
```

```powershell
# PowerShell 7+ (Windows / macOS / Linux)
./list-ai-models.ps1 -Subscription <your-subscription-id> -Location eastus2
```

Example output (one block per tier):

```
  FAST tier  → the "Azure AI Fast Model" field
    MODEL                      VERSION           LIMIT       USED       FREE   VALUE TO PASTE
    gpt-4.1-mini               2025-04-14       225000          0     225000   gpt-4.1-mini@2025-04-14
    gpt-4o-mini                2024-07-18       225000          0     225000   gpt-4o-mini@2024-07-18
```

`LIMIT` / `USED` / `FREE` are in **thousands of TPM** — the same unit as the *Azure AI
Capacity* fields, so the capacity you request for a tier must be **≤ its `FREE`**
value. The `@version` suffix is optional; drop it to let Azure pick the current default.
Only models that are both deployable in the region **and** carry quota are listed.

### Options

| Flag (bash) | PowerShell | Description | Default |
|------|------|-------------|---------|
| `-s`, `--subscription` | `-Subscription` | Target Azure subscription ID | _(required)_ |
| `-l`, `--location` | `-Location` | Azure region to check (quota is per-region) | _(required)_ |
| `--sku` | `-Sku` | Deployment SKU to report quota for | `GlobalStandard` |

If a tier shows no candidates with quota, request a quota increase for the model you
want (Azure AI Foundry → Management center → Quota) or pick one that already has quota.
...
```
azure-ent-deploy-role/
├── setup.sh            # bash (macOS / Linux) — create the deploy identity
├── setup.ps1           # PowerShell 7+ — create the deploy identity
├── list-ai-models.sh   # bash — list AI model candidates + TPM quota (read-only)
├── list-ai-models.ps1  # PowerShell 7+ — list AI model candidates + TPM quota (read-only)
├── .gitignore
└── README.md
```
